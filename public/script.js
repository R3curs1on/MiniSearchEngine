  ...
  try {
    ...
    if (!data.results || data.results.length === 0) {
      ...
      setBadges([
        ...
        ...(data.diagnostics?.corrected_query
          ? [{ label: "Showing for", value: data.diagnostics.corrected_query }]
          : []),
      ]);
      return;
    }
    ...
    const matchedTerms = joinOrNone(data.diagnostics?.matched_terms || []);
    const searchQuery = data.search_query || query;
    const correctedLine = data.diagnostics?.correction_applied
    ...
    setBadges([
      ...
      { label: "Search terms", value: joinOrNone(data.diagnostics?.search_terms || []) },
      ...
      ...(data.diagnostics?.corrected_query
        ? [{ label: "Corrected", value: data.diagnostics.corrected_query }]
        : []),
    ]);
    ...
    error.textContent = "Could not reach API. Is the backend running?";
...
function createResultCard(result, index) {
  ...
  [
    ...
    { key: "Phrase", value: result.signals?.phrase },
    { key: "Proximity", value: result.signals?.proximity },
  ].forEach((item) => {
